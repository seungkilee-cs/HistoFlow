...
def train_classifier():
    """Train a DINOv2 + SVM (RBF) classifier on PCam dataset"""
    ...
    print("=" * 60)
    print("TRAINING DINOV2 + SVM CLASSIFIER")
    print("=" * 60)
    ...
    # Build Pipeline: StandardScaler + SVM (RBF kernel)
    # SVM with RBF kernel is sensitive to feature scale; scaler is bundled
    # into the pipeline so the saved .pkl handles normalization automatically.
    print("\nTraining StandardScaler + SVM (RBF) pipeline...")
    clf = Pipeline([
    ...
    # Save the model
    model_path = Path("models/dinov2_svm.pkl")
